...
test "read QoiHeader" {
    ...
    try std.testing.expectError(zigqoi.QoiHeader.HeaderDecodingError.WrongFiletype, zigqoi.QoiHeader.from_bytes(png_file));
}
...
test "read QoiImage" {
    ...
    try std.testing.expectError(zigqoi.QoiHeader.HeaderDecodingError.WrongFiletype, zigqoi.QoiImage.from_bytes(alloc, png_file));
}

test "read QoiImage OutOfMemory" {
    const fail_alloc = std.testing.failing_allocator;
    try std.testing.expectError(zigqoi.QoiImage.QoiDecodingError.OutOfMemory, zigqoi.QoiImage.from_bytes(fail_alloc, four_by_four));
}
